...
// Dedikované stránky pro kraje — zachytí vše v marketingu bez ohledu na keyword
const REGION_URLS = [
  "https://www.jobs.cz/prace/plzensky-kraj/marketing/",
  "https://www.jobs.cz/prace/hlavni-mesto-praha/marketing/",
];
...
async function fetchPage(keyword) {
  ...
  return fetchUrl(`https://www.jobs.cz/prace/?q[]=${encoded}`);
}
...
export async function scrapeJobsCz() {
  ...
  // 2. Krajové stránky — vše z marketingu v daném kraji
  const regionResults = await Promise.allSettled(REGION_URLS.map((url) => fetchUrl(url).then(parseJobs)));
